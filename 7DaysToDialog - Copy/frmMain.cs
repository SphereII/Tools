            ...
            using(SaveFileDialog saveFileDialog = new SaveFileDialog())
            {
              ...
              //  saveFileDialog.RestoreDirectory = true;
                ...
                if(saveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    String DeployFolder = Path.GetDirectoryName( saveFileDialog.FileName );
                    String strLocalizeFile = Path.Combine(DeployFolder, "Localization.txt");
                    if(File.Exists(strLocalizeFile))
                    {
                        Utilities.InitLocalization(strLocalizeFile);
                    }
                    foreach(NPC npc in NPCs)
                    {
                        // 
                        ...
                        {
                            String StatementText = statement.Value.Text;
                            String strKey = "dialog_" + npc.Name + "_" + statement.Value.ID;
                            Utilities.AddToLocalization(strKey, StatementText);
                        ...
                        foreach(KeyValuePair<string, Response> response in npc.Responses)
                        {
                            ...
                            String strKey = "response_" + npc.Name + "_"+ response.Value.ID;
                            Utilities.AddToLocalization(strKey, Text);

                            npcNode.AppendChild(response.Value.GenerateXMLNode(saveDoc));
                        }
                        saveDoc.Save(saveFileDialog.FileName);
                        Utilities.WriteLocalization(strLocalizeFile);
                    }
                }
            }
            SelectedNPC = null;
